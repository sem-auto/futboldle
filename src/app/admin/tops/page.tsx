...
export default function AdminTopsPage() {
  ...
  function statusFor(id: string) {
    ...
    if (id === dailyTop.id) return { label: "Diario", color: "#c8920a", bg: "#fff8e6" };
    return { label: "Publicado", color: "#6b6b72", bg: "#f3efe8" };
  }
    ...
    <main className="min-h-dvh px-3 py-4" style={{ background: "#f6f2ea" }}>
      <div className="max-w-3xl mx-auto flex flex-col gap-4">
        <div className="flex items-center justify-between">
          <Link href="/admin/audit" className="text-[11px] font-semibold" style={{ color: "#6b6b72" }}>{"<-"} Auditoria</Link>
          <Link href="/" className="text-[11px] font-semibold" style={{ color: "#c8920a" }}>Home</Link>
        </div>

        <section className="rounded-2xl px-5 py-4" style={{ background: "white", border: "1px solid rgba(0,0,0,0.08)" }}>
          <div className="text-[9px] font-semibold uppercase tracking-[0.2em]" style={{ color: "#1a4fa0" }}>Admin interno</div>
          <h1 className="font-bebas text-[36px] leading-none" style={{ color: "#18181b" }}>CATALOGO DE TOPS</h1>
          <p className="text-[12px]" style={{ color: "#9a9a8a" }}>Vista interna de Top10 publicados y diario actual.</p>
        </section>
