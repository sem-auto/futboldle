...
function repairTop10Text(value: string) {
  if (!/[\u00c2-\u00f0\u2013-\u2122]/.test(value)) return value;
  let current = value;
  for (let pass = 0; pass < 2; pass++) {
    if (!/[\u00c2\u00c3\u00c4\u00e2\u00f0]/.test(current)) break;
    const bytes: number[] = [];
    let valid = true;
...
for (const challenge of top10Challenges) {
  ...
  challenge.period = repairTop10Text(challenge.period);
  challenge.criterion = repairTop10Text(challenge.criterion);
  challenge.source = repairTop10Text(challenge.source);
  challenge.sourceName = repairTop10Text(challenge.sourceName) as Top10Challenge["sourceName"];
  challenge.sourceNote = repairTop10Text(challenge.sourceNote);
  challenge.title = repairTop10Text(challenge.title);
  challenge.subtitle = repairTop10Text(challenge.subtitle);
  challenge.consigna = repairTop10Text(challenge.consigna);
  challenge.emoji = repairTop10Text(challenge.emoji);
  for (const answer of challenge.answers) {
    answer.displayName = repairTop10Text(answer.displayName);
    answer.detail = repairTop10Text(answer.detail);
    if (answer.label) answer.label = repairTop10Text(answer.label);
    answer.hintNationality = repairTop10Text(answer.hintNationality);
