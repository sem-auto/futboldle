...
export default function PwaControls() {
  ...
  return <section className="rounded-xl p-4" style={{ background: "white", border: "1px solid rgba(0,0,0,0.08)" }}><div className="font-bebas text-[22px]">FUTBOLDLE EN TU MOVIL</div><p className="text-[11px] mt-1" style={{ color: "#6b6b72" }}>Instala la web y permite avisos del navegador cuando volvamos a activarlos.</p><div className="flex flex-wrap gap-2 mt-3"><button onClick={install} className="rounded-lg px-3 py-2 text-[10px] font-semibold" style={{ background: "#18181b", color: "white" }}>Instalar Futboldle</button><button onClick={notifications} disabled={permission === "granted"} className="rounded-lg px-3 py-2 text-[10px] font-semibold" style={{ background: "#eef3ff", color: "#174ea6" }}>{permission === "granted" ? "Avisos permitidos" : "Permitir avisos"}</button></div>{message && <div className="text-[10px] mt-2" style={{ color: "#1e6b2e" }}>{message}</div>}</section>;
}
