...
import { trackChallengeCompleted, trackChallengeShared, trackChallengeStarted, trackEvent, trackModeEntered } from "@/lib/analytics";
...
export default function Mundialdle({ onBack }: { onBack?: () => void }) {
  ...
  function share() {
    ...
    const text = `Mundialdle #${getDayNumber()}\n${score}\n🌍${rows}\n\nfutboldle.es`;
    trackChallengeShared("mundialdle", challenge.id, { seasonId: "world-cups", won, attempts: guesses.length });
    shareResult(text, () => { setCopied(true); setTimeout(() => setCopied(false), 1800); }, "Mundialdle");
  }
