...
export default function FinalMundial() {
  ...
  function answer(year: number) {
    ...
    if (correct && reward) {
      ...
      if (unlocked) {
        trackEvent("card_unlocked", { seasonId: "world-cups", modeId: "final-mundial", playerId: reward.id });
        window.dispatchEvent(new CustomEvent("fbl-card-unlocked", {
          detail: {
            name: reward.name,
            rarity: reward.iconicLevel === "icono" ? "Leyenda" : reward.iconicLevel === "legendario" ? "Legendario" : reward.iconicLevel === "core" ? "Estrella" : "Culto",
            clubs: reward.clubsByWorldCup?.map(item => item.club) ?? [],
            position: reward.position,
            source: "Final Mundial",
            season: `Mundial ${challenge.year}`,
  ...
  function share() {
    const text = [`Final Mundial #${dayNumber}`, won ? "Correcto en 1 intento" : "Hoy no salió", "Final reconocida: " + (won ? "Sí" : "No"), "¿Puedes superarme?", FUTBOLDLE_URL].join("\n");
